...
class ArduinoEspPlatform {
  constructor(log, config, api) {
    ...
    this.api.on('didFinishLaunching', async () => {
      try {
        if (!this.hasUsableConfig()) {
          this.log.warn(`[${PLATFORM_NAME}] Plugin is installed but not fully configured yet. Add a Server URL and at least one accessory before starting.`);
          return;
        }
        ...
        if (this.bindings.size === 0) {
          this.log.warn(`[${PLATFORM_NAME}] No valid accessories were created from the current configuration. Polling will not start.`);
          return;
        }
  ...
  hasUsableConfig() {
    const serverUrl = String(this.config.serverurl || '').trim();
    ...
    return devices.some((device) => Array.isArray(device.accessories) && device.accessories.length > 0);
  }
