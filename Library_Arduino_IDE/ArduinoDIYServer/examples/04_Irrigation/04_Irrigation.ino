...
const char* OTA_HOSTNAME = "irrigation-device";
...
const uint8_t RELAY_PIN[] = {D1, D2, D5, D6};
const char* VPIN_VALVE[] = {"V4", "V5", "V6", "V8"};
const int valveCount = sizeof(VPIN_VALVE) / sizeof(VPIN_VALVE[0]);
...
void loop() {
  ...
  if (millis() - lastPollMs >= POLL_INTERVAL_MS) {
    lastPollMs = millis();
		...
		for (int i = 0; i < valveCount; i++) {
			pollVpinFromServer(VPIN_VALVE[i]);
			...
			updateVpinFromServer(VPIN_VALVE[i]);
		}
  }
}
